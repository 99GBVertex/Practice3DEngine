...
void LveSwapChain::createImageViews() {
  ...
  for (size_t i = 0; i < swapChainImages.size(); i++) {
	  device.createImageView(swapChainImages[i], swapChainImageFormat, VK_IMAGE_ASPECT_COLOR_BIT, swapChainImageViews[i]);
  }
}
...
void LveSwapChain::createDepthResources() {
  ...
  for (int i = 0; i < depthImages.size(); i++) {
	...
	device.createImageView(depthImages[i], depthFormat, VK_IMAGE_ASPECT_DEPTH_BIT, depthImageViews[i]);
  }
}
